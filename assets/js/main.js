(function () {
  ...
  var FINAL_CURSOR = '<span class="prompt-line">guest@antoniocali:~$ <span class="caret"></span></span>';

  function renderStatic() {
    var html = "";
    script.forEach(function (line) {
      if (line.type === "prompt") {
        html += '<span class="prompt-line">guest@antoniocali:~$ ' + line.text + "</span>\n";
      } else {
  ...
  function typeNext() {
    ...
    var prefix = line.type === "prompt" ? "guest@antoniocali:~$ " : "";
  ...
  function tick() {
    ...
    for (var a = 0; a < particles.length; a++) {
      for (var b = a + 1; b < particles.length; b++) {
        ...
        if (dist < LINK_DIST) {
          ctx.strokeStyle = "rgba(63, 185, 80, " + (0.12 * (1 - dist / LINK_DIST)) + ")";
          ctx.lineWidth = 1;
          ctx.beginPath();
